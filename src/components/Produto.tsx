...
export function Produto({imagem, descricao, preco, medida, qtdEstoque, produtor} : ProdutoProps) {
    ...
    const mudarBotao = (event : React.MouseEvent<HTMLButtonElement>) => {
        const botaoAddItem = event.currentTarget.parentElement?.querySelector(`.${styles.botaoAddItem}`) as HTMLDivElement;
    ...
    const removerItem = (event : React.MouseEvent<HTMLButtonElement>) => {
        if(quantidade>1)
            setQuantidade(quantidade-1);
        if(quantidade==1){
            const botaoAddItem = event.currentTarget.parentElement?.parentElement?.querySelector(`.${styles.botaoAddItem}`) as HTMLDivElement;
